...
pub(crate) struct MessageQueue<T, const N: usize> {
...
impl<T, const N: usize> MessageQueue<T, N> {
    ...
    pub fn send_cs(&self, cs: &CriticalSection, msg: T) -> SendResult<T> {
    ...
    pub fn try_recv_cs(&self, cs: &CriticalSection) -> Option<T> {
        self.inner.lock(cs, |inner| {
            let msg = inner.buf.pop_front();
    ...
    pub fn try_send_cs(&self, cs: &CriticalSection, msg: T) -> bool {
        self.inner.lock(cs, |inner| match inner.buf.push_back(msg) {
            Ok(()) => {
                inner.recv_waiters.wake_one(cs);
                true
